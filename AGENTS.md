...
These instructions have been created to help Cursor but are expected to be generally
helpful to any coding agent. They are copied from dds, this repository's one
dependency, so that the two trees read the same way; see
...
Exceptions:
- Match existing external or legacy APIs. This repository writes no such API of
  its own, but it calls dds's, which has names in a different style and in
  British spelling (for example `AnalysePlay*`, `DDS_STRAINS`). Spell those the
  way dds spells them.
