// safety_score_declarer_play() (test_support.hpp) is the "real work" pi
// python_cost_timer(_test).{cpp,py} time against single_card_declarer_play's
// O(1) rule -- its own scoring is what makes that comparison mean anything,
// so it gets a hand-derived correctness check independent of evaluate() and
// of any fixture (oracle_test.cpp's own precedent for why a small enough
// case is safe to hand-derive rather than build a "same construction, once"
// generator for: a transcription slip here would show up as *some other
// score*, not as an agreement with the wrong one). The Python port in
...
//
// The Ace's score is strictly lower -- safety_score_declarer_play must
// return it, not the Five lowest_legal_card would.
TEST(SafetyScoreDeclarerPlayTest, PlaysTheCardNoDefenderLayoutCanBeat)
{
    be::ObservationState state{};
    state.declarer = North;
    state.known_holdings.first = North;
    state.known_holdings.remainCards[North][Spades] = be::holding({5, 14});

    Deal layout_a{};
    layout_a.remainCards[East][Spades] = be::holding({13});
    ...
    be::Card const played = be::safety_score_declarer_play(state, view);
...
TEST(SafetyScoreDeclarerPlayTest, ReturnsTheOnlyLegalCardWhenThereIsNoChoice)
{
    be::ObservationState state{};
    state.declarer = North;
    state.known_holdings.first = North;
    state.known_holdings.remainCards[North][Spades] = be::holding({5});
...
TEST(SafetyScoreDeclarerPlayTest, TiesBreakByLowestRank)
{
    be::ObservationState state{};
    state.declarer = North;
    state.known_holdings.first = North;
